...
import EditModal from '../ErrorCategory/edit'
...
function ErrorCategory() {
  ...
  const onSubmit = async (e) => {
    ...
    e.preventDefault();
    await axios
      .post(`${config.API_URL}//errorCategory`, formData)
      .then((res) => {
        if (res) {
          // alert('done')
          console.log("Response :");
  ...
  const onDelete = async (id) => {
    ...
    if (DELETE) {
      setTimeout(async () => {
        await axios
        .delete(`${config.API_URL}//errorCategory/${id}`)
        .then((response) => {
    ...
    <EditModal
      ...
      <Col md="12">
          ...
          <Col md="12">
            <Card>
              <CardHeader>
                <CardTitle tag="h4">Error Category </CardTitle>
              </CardHeader>
